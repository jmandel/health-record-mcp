    ...
    <div className="question-card">
      <div className="card-content">
        ...
        {(question.questionType === 'boolean' || question.questionType === 'multipleChoice') && (
            <div>
              {question.options?.map((o, i) => {
                 ...
                 // Apply conditional class if meetsCriteria is true
        ...
        {question.questionType === 'freeText' && (
            // --- Conditionally render main input vs. snippet input --- 
            !shouldShowSnippetEditor ? (
              // Case 1: Snippet editor is HIDDEN - show only the main input
            <div className="form-group">
                <textarea 
                    value={inputValue}
                    onChange={handleInputChange}
              ...
              // Case 2: Snippet editor is SHOWN - show *only* the snippet editor
              // (The main input textarea is omitted here)
              <div className="form-group snippet-container"> {/* Combine form-group and snippet-container */} 
                  <p className="snippet-title">Response / Optional Note:</p> {/* Adjusted title */} 
                     <textarea
                         ref={textareaRef}
                      className="snippet-textarea snippet-textarea-optional" // Keep optional class?
                         value={snippetContent}
                      onChange={handleSnippetChange} // Edits here update snippetContent
                      placeholder="Enter response here..." // Adjusted placeholder
                     />
                 </div>
            )
        )}
...
const Orders2Tab: React.FC = () => {
  ...
  const handleAnswer = useCallback((questionId: string, value: string, snippetContent: string) => {
    ...
    const isReadyToSubmit = useMemo(() => {
      ...
      // Check if every question has either a non-empty value OR
      // (for freeText questions where snippet is the input) a non-empty snippet.
      return currentQuestions.every(q => {
        const answer = userAnswers[q.id];
        const hasValue = !!answer?.value?.trim();
        const hasSnippet = !!answer?.snippet?.trim();

        // Considered answered if value exists OR if it's freeText and only snippet exists
...
const extractPatientAdminDetails = (ehrData: { fhir?: Record<string, any[]> } | null | undefined): string => {
